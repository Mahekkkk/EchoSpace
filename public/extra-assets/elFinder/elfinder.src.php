            ...
            $('#elfinder').elfinder(
                // 1st Arg - options
                {
                    ...
                    getFileCallback: function(file, fm) {
                        let fileURL = file.url; // Correct assignment with `=`
                        let imageFullPath = new URL(fm.convAbsUrl(fileURL)); // Fix function name typo and parentheses
                        let relImagePath = imageFullPath.pathname; // Correct assignment with `=`
                        
                        // Extract CKEditor function number
                        var funcNum = window.location.search.replace(/^.*CKEditorFuncNum=(\d+).*$/, "$1");
                        // Extract language code
                        var langCode = window.location.search.replace(/^.*langCode=([a-z]{2}).*$/, "$1");
                        // Pass the image path to CKEditor
                        window.opener.CKEDITOR.tools.callFunction(funcNum, relImagePath);
                        // Clean up and close the file manager
                        fm.destroy();
                        window.close();
                    },
                },

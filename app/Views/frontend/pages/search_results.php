...
<div class="row">
    <div class="col-12">
        <h1 class="mb-4 border-bottom border-primary d-inline-block">Search Results for: <?= esc($query) ?></h1>
    </div>
                        ...
                        <article class="card article-card article-card-sm h-100">
                            <a href="<?= route_to('read-post', $post->slug) ?>">
                                <div class="card-image">
                                    ...
                                    <img loading="lazy" decoding="async" src="/images/posts/<?= esc($post->featured_image) ?>" alt="Post Thumbnail" class="w-100" width="420" height="280">
                                </div>
                            </a>
                            <div class="card-body px-0 pb-0">
                                <h2>
                                    <a class="post-title" href="<?= route_to('read-post', $post->slug) ?>"><?= esc($post->title) ?></a>
                                </h2>
                                <p class="card-text"><?= limit_words($post->content, 13); ?></p>
                                <div class="content">
                                    <a class="read-more-btn" href="<?= route_to('read-post', $post->slug) ?>">Read Full Article</a>
                                </div>
                            </div>
                        </article>
                    </div>
